...
//	feats.add("firstName5=" + firstName.charAt(5));

    for(int i = 0; i < Math.min(firstName.length(), 5); i++){
	    String idx = Integer.toString(i);
	    feats.add("firstName" + idx + "=" + firstName.charAt(i));
    }
    for(int i = 0; i < Math.min(lastName.length(), 5); i++){
        String idx = Integer.toString(i);
        feats.add("lastName" + idx + "=" + lastName.charAt(i));
    }
